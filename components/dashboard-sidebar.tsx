...
export default function DashboardSidebar({
  ...
  return (
    ...
    >
      <div className="flex min-h-0 min-w-0 flex-1 flex-col px-4 py-5 min-[1200px]:px-5 min-[1200px]:py-6">
        <nav className="min-w-0 space-y-2">
          {dashboardNavItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={
                  isActive
                    ? "relative flex min-h-12 min-w-0 items-center gap-3 rounded-2xl bg-gradient-to-r from-violet-700 via-violet-600 to-fuchsia-600 px-3 py-3 text-sm font-medium text-white shadow-[0_20px_44px_-24px_rgba(109,40,217,0.82)] ring-1 ring-violet-300/40 before:absolute before:inset-0 before:-z-10 before:rounded-2xl before:bg-violet-500/30 before:blur-xl min-[1200px]:px-4"
                    : "flex min-h-12 min-w-0 items-center gap-3 rounded-2xl px-3 py-3 text-sm font-medium text-violet-950/72 transition duration-300 hover:bg-violet-50 hover:text-violet-900 hover:shadow-[0_14px_32px_-22px_rgba(109,40,217,0.22)] min-[1200px]:px-4"
                }
              >
        ...
        <div className="mt-auto min-h-0 min-w-0 space-y-4 pt-5">
          <SidebarBankingSection accounts={accounts} demoMode={demoMode} />

          <div className="border-t border-violet-100 pt-5">
            <p className="break-words text-xs leading-6 text-violet-950/55">
