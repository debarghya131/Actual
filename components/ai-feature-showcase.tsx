...
const aiSlides = [
  ...
  {
    image: "/ai/ai2.png",
    title: "Chat And Insights Modes",
    description:
      "Switch between focused insight themes and direct chat with Kubera from a dedicated AI navigation panel.",
  },
  {
    image: "/ai/ai3.png",
    title: "Personal Finance Answers",
    description:
      "Answer user prompts with finance-aware responses, such as identifying the category where overspending is happening fastest.",
  },
  {
    image: "/ai/ai4.png",
    title: "Insight Theme Cards",
    description:
      "Organize AI guidance into clear themes like savings summary, spending pattern detection, and future month prediction.",
  },
  {
    image: "/ai/ai5.png",
    title: "Starter Prompts",
    description:
      "Help users begin quickly with practical prompts for overspending, budgeting, and saving more consistently.",
  },
];

export default function AiFeatureShowcase() {
  const [activeIndex, setActiveIndex] = useState(0);
  const activeSlide = aiSlides[activeIndex];

  const showPrevious = () => {
    setActiveIndex((current) =>
      current === 0 ? aiSlides.length - 1 : current - 1
    );
  };

  const showNext = () => {
    setActiveIndex((current) =>
      current === aiSlides.length - 1 ? 0 : current + 1
    );
  };
    ...
    <FeatureShowcaseMotion className="mt-14 grid min-w-0 items-start gap-6 sm:mt-16 sm:gap-8 lg:mt-20 lg:grid-cols-[minmax(18rem,0.92fr)_minmax(0,1.08fr)] lg:items-center lg:gap-10 xl:gap-12">
      ...
      <div className="feature-showcase-card min-w-0 rounded-2xl border border-violet-100/80 bg-white/88 p-2 shadow-[0_26px_70px_-42px_rgba(91,33,182,0.38)] backdrop-blur sm:rounded-3xl sm:p-4">
        <div className="relative flex aspect-[1.08/1] items-center justify-center overflow-hidden rounded-xl border border-violet-100 bg-[linear-gradient(135deg,_rgba(250,245,255,0.92),_rgba(255,255,255,0.98))] p-2 min-[420px]:aspect-[4/3] sm:aspect-[16/10] sm:rounded-2xl sm:p-5">
          ...
          <button
            type="button"
            aria-label="Show previous AI image"
